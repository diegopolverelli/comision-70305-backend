...
db.heroes.updateMany({team:"Avengers"}, {$set:{selected:true}})
...
db.heroes.find({}, {_id:0, name:1, publisher:1})
...
// 2)
db.estudiantes.find({},{_id:0, nombre:1, edad:1}).sort({edad:-1})

// 3)
db.estudiantes.find({edad:{$exists:true}},{_id:0, nombre:1, edad:1}).sort({edad:1}).limit(1)

// 4)
db.estudiantes.find({edad:{$exists:true}},{_id:0, nombre:1, edad:1}).sort({edad:1}).limit(1).skip(1)

// 7)
db.estudiantes.find({nombre:{$in:["Juan", "Lucia"]}})
db.estudiantes.find({$or:[{nombre:"Juan"}, {nombre:"Lucia"}]})

// 10)
db.estudiantes.find({edad:{$ne:25}}, {_id:0, nombre:1, edad:1})
